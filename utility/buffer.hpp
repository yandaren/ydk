...
public:
    ...
    buffer(int32_t size, bool auto_extend = false)
        : data_(0)
        , capacity_(0)
        , reader_index_(0)
        , writer_index_(0)
    ...
    static buffer_ptr create(int32_t size, bool auto_extend = false)
    {
        return std::make_shared<buffer>(size, auto_extend);
    }
...
public:
	buffer_ptr copy() {
		buffer_ptr buf = create(readable_bytes());
		buf->write_bytes(data(), readable_bytes());
		return buf;
	}
    ...
    int32_t read_bytes(buffer_ptr dst, int32_t length){
        int32_t to_read_len = length;
        if (to_read_len > readable_bytes()){
            to_read_len = readable_bytes();
        }
